...
class TestBM25Retriever:
    ...
    def test_tokenize(self, sample_documents, stopwords_file, temp_index_path):
        ...
        retriever = BM25Retriever(
            docs=sample_documents, stopwords_path=stopwords_file, retrieve=False
        )
    ...
    def test_retrieve_topk(self, sample_documents, temp_index_path):
        ...
        if len(results) > 0:
            ...
            # 检查第一个结果是否包含查询词或相关词
            query_in_first = any(
                token in first_doc_content for token in query_tokens if len(token) > 1
            )
            assert (
                query_in_first
                or "车窗" in first_doc_content
                or "打开" in first_doc_content
            ), (
    ...
    def test_stopwords_filtering(
        self, sample_documents, stopwords_file, temp_index_path
    ):
        """
        测试：停用词过滤功能
