...
OUTPUT_BASE_DIR="rag_test_reports/finetuned_evaluation"
...
if [ -f "scripts/analyze_evaluation_results.py" ]; then
    ...
    # 微调模型的文件名是 finetuned_evaluation_results.json 和 finetuned_evaluation_summary.json
    SUMMARY_FILE="${OUTPUT_DIR}/finetuned_evaluation_summary.json"
    RESULTS_FILE="${OUTPUT_DIR}/finetuned_evaluation_results.json"
    
    if [ -f "${SUMMARY_FILE}" ] && [ -f "${RESULTS_FILE}" ]; then
        python scripts/analyze_evaluation_results.py \
            "${SUMMARY_FILE}" \
            "${RESULTS_FILE}" \
            > "${OUTPUT_DIR}/finetuned_analysis.txt" 2>&1 || true
    else
...
echo "  - 评估结果: ${OUTPUT_DIR}/finetuned_evaluation_results.json"
echo "  - 汇总指标: ${OUTPUT_DIR}/finetuned_evaluation_summary.json"
echo "  - 评估日志: ${OUTPUT_DIR}/finetuned_evaluation.log"
if [ -f "${OUTPUT_DIR}/finetuned_analysis.txt" ]; then
    echo "  - 分析报告: ${OUTPUT_DIR}/finetuned_analysis.txt"
fi
echo ""
