...
def chat_non_stream(
    ...
    try:
        response = requests.post(f"{rag_url}/chat", json=request_data, timeout=120)
            ...
            if isinstance(img_info, dict):
                ...
                if img_path:
                    ...
                    if checked_path:
                        ...
                        print(
                            f"DEBUG: 成功添加图片 - 路径: {checked_path}, 标题: {title}"
                        )
                    else:
                        print(f"DEBUG: 图片路径无效 - {img_path}")
                else:
                    print("DEBUG: 图片信息中没有路径字段")
            ...
            if line.startswith("data: "):
                ...
                try:
                    ...
                    if data.get("type") == "token":
                        ...
                        # 格式化性能指标
                        performance_text = format_performance_metrics(
