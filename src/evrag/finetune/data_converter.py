...
class DataConverter:
    ...
    def create_dataset_config(
        ...
        train_file_str = (
            str(train_file.absolute()) if train_file.is_absolute() else str(train_file)
        )
        dataset_config = {
            dataset_name: {
        ...
        if test_file and test_file.exists():
            test_file_str = (
                str(test_file.absolute()) if test_file.is_absolute() else str(test_file)
            )
        ...
        # 使用相对于data目录的路径
        train_file_relative = (
            str(train_file.relative_to(Path("data")))
            if Path("data") in train_file.parents
            else str(train_file)
        )
        ...
        if test_file and test_file.exists():
            test_file_relative = (
                str(test_file.relative_to(Path("data")))
                if Path("data") in test_file.parents
                else str(test_file)
            )
