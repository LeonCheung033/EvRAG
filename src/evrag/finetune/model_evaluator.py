...
class ModelEvaluator:
    ...
    def _calculate_bleu(self, predicted: str, expected: str) -> float:
        ...
        try:
            ...
            if NLTK_AVAILABLE:
                ...
                score = sentence_bleu(
                    [exp_tokens], pred_tokens, smoothing_function=smoothing
                )
                return float(score)
            else:
                ...
                # 计算1-gram和2-gram的重叠率
                pred_bigrams = (
                    set(zip(pred_tokens, pred_tokens[1:]))
                    if len(pred_tokens) > 1
                    else set()
                )
                exp_bigrams = (
                    set(zip(exp_tokens, exp_tokens[1:]))
                    if len(exp_tokens) > 1
                    else set()
                )
                ...
                unigram_overlap = (
                    len(pred_unigrams & exp_unigrams) / len(exp_unigrams)
                    if exp_unigrams
                    else 0.0
                )
                bigram_overlap = (
                    len(pred_bigrams & exp_bigrams) / len(exp_bigrams)
                    if exp_bigrams
                    else 0.0
                )
    ...
    def _calculate_rouge(self, predicted: str, expected: str) -> Dict[str, float]:
        ...
        try:
            # 判断是否为中文文本
            is_chinese = self._is_chinese_text(predicted) or self._is_chinese_text(
                expected
            )
    ...
    def _calculate_rouge_chinese(
        ...
        try:
            ...
            if len(exp_unigrams) == 0:
                ...
                precision_1 = (
                    len(common_unigrams) / len(pred_unigrams)
            ...
            if len(pred_tokens) < 2 or len(exp_tokens) < 2:
                ...
                if len(exp_bigrams) == 0:
                    ...
                    precision_2 = (
                        len(common_bigrams) / len(pred_bigrams)
                        if len(pred_bigrams) > 0
                        else 0.0
                    )
                    recall_2 = len(common_bigrams) / len(exp_bigrams)
                    rouge2 = (
                        2 * precision_2 * recall_2 / (precision_2 + recall_2)
    ...
    def _batch_chat_vllm(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=min(len(batch_requests), 16)
        ) as executor:
            # 提交所有请求，并记录索引
            future_to_idx = {
    ...
    def _evaluate_llm_with_vllm(
        ...
        for batch_idx in range(0, len(prompts), batch_size):
            ...
            print(
                f"  处理批次 {batch_idx // batch_size + 1}/{(len(prompts) - 1) // batch_size + 1} "
            ...
            tokens_generated = sum(
                len(resp) // 3 for resp in batch_responses
            )  # 粗略估算
            tokens_per_sec = tokens_generated / batch_time if batch_time > 0 else 0
            print(
                f"    批次完成（{batch_time:.2f}秒，约{tokens_generated} tokens，{tokens_per_sec:.1f} tokens/s）"
            )
        ...
        print(
            f"  ✓ 所有样本生成完成（总时间: {total_time:.2f}秒，平均: {total_time / total_samples:.2f}秒/样本）"
        )

        # 计算指标
        print("  正在计算指标...")
        for idx, (item, predicted_text) in enumerate(
            zip(items_data, predicted_texts), 1
        ):
        ...
        results["metrics"] = {
            ...
            "citation_accuracy": np.mean(
                [p["citation_accuracy"] for p in results["predictions"]]
            ),
        }
        print("  ✓ 指标计算完成")
    ...
    def evaluate_reranker(
        ...
        for idx, (query, items) in enumerate(query_groups.items(), 1):
            if idx % 50 == 0 or idx == 1:
                print(
                    f"  处理查询 {idx}/{total_queries} (已处理 {processed_items}/{total_items} 个文档对)..."
                )
        ...
        results["metrics"] = {
            ...
            f"precision@{topk}": np.mean(all_precision_scores)
        ...
        print(
            f"✓ Reranker评估完成: {results['total_samples']} 个样本, {len(query_groups)} 个查询"
        )
