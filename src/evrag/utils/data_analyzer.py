...
def analyze_documents(
    ...
    # 显示统计信息
    table = Table(
        title=f"{stage_name} 统计信息", show_header=True, header_style="bold magenta"
    )
    ...
    table.add_row(
        "包含parent_id",
        f"{has_parent_id} ({has_parent_id / total_docs * 100:.1f}%)"
        if total_docs > 0
        else "0",
    )
    table.add_row(
        "包含图片",
        f"{has_images} ({has_images / total_docs * 100:.1f}%)"
        if total_docs > 0
        else "0",
    )
    if page_numbers:
    ...
    # 显示长度分布
    dist_table = Table(
        title=f"{stage_name} 长度分布", show_header=True, header_style="bold blue"
    )
...
def check_duplicates(docs: List[Document]) -> Dict[str, Any]:
    """检查重复文档"""
    unique_ids = [
        doc.metadata.get("unique_id") for doc in docs if doc.metadata.get("unique_id")
    ]
...
def analyze_parent_child_relationship(split_docs: List[Document]) -> Dict[str, Any]:
    ...
    return {
        ...
        "avg_children_per_parent": sum(children_per_parent.values())
        / len(children_per_parent)
        if children_per_parent
        else 0,
        "max_children_per_parent": max(children_per_parent.values())
        if children_per_parent
        else 0,
        "min_children_per_parent": min(children_per_parent.values())
        if children_per_parent
        else 0,
    }
...
def compare_stages(
    ...
    # 分析各阶段
    raw_stats = analyze_documents(
        raw_docs, "原始文档 (Raw)", show_samples=True, num_samples=2
    )
    clean_stats = analyze_documents(
        clean_docs, "清洗后文档 (Clean)", show_samples=True, num_samples=2
    )
    split_stats = analyze_documents(
        split_docs, "切分后文档 (Split)", show_samples=True, num_samples=3
    )
    ...
    console.print(
        f"原始文档 - 唯一ID数: {raw_duplicates['total_unique_ids']}, 重复ID: {raw_duplicates['duplicate_ids']}"
    )
    console.print(
        f"清洗后文档 - 唯一ID数: {clean_duplicates['total_unique_ids']}, 重复ID: {clean_duplicates['duplicate_ids']}"
    )
    console.print(
        f"切分后文档 - 唯一ID数: {split_duplicates['total_unique_ids']}, 重复ID: {split_duplicates['duplicate_ids']}"
    )

    if split_duplicates["duplicate_ids"] > 0:
        console.print(
            f"[yellow]警告: 发现 {split_duplicates['duplicate_ids']} 个重复的unique_id[/yellow]"
        )
    ...
    rel_table = Table(
        title="父子文档关系", show_header=True, header_style="bold magenta"
    )
    ...
    rel_table.add_row(
        "平均每个父文档的子文档数", f"{relationship['avg_children_per_parent']:.2f}"
    )
    ...
    compare_table = Table(
        title="三个阶段对比", show_header=True, header_style="bold blue"
    )
    ...
    compare_table.add_row(
        "文档总数",
        str(raw_stats["total_docs"]),
        str(clean_stats["total_docs"]),
        str(split_stats["total_docs"]),
    )
    compare_table.add_row(
        "平均字符数",
        f"{raw_stats['avg_char_count']:.0f}",
        f"{clean_stats['avg_char_count']:.0f}",
        f"{split_stats['avg_char_count']:.0f}",
    )
    compare_table.add_row(
        "平均Token数",
        f"{raw_stats['avg_token_count']:.0f}",
        f"{clean_stats['avg_token_count']:.0f}",
        f"{split_stats['avg_token_count']:.0f}",
    )
    compare_table.add_row(
        "最大字符数",
        str(raw_stats["max_char_count"]),
        str(clean_stats["max_char_count"]),
        str(split_stats["max_char_count"]),
    )
    compare_table.add_row(
        "最小字符数",
        str(raw_stats["min_char_count"]),
        str(clean_stats["min_char_count"]),
        str(split_stats["min_char_count"]),
    )
    ...
    # 计算切分效果
    expansion_ratio = (
        split_stats["total_docs"] / clean_stats["total_docs"]
        if clean_stats["total_docs"] > 0
        else 0
    )
    avg_size_reduction = (
        (1 - split_stats["avg_char_count"] / clean_stats["avg_char_count"]) * 100
        if clean_stats["avg_char_count"] > 0
        else 0
    )

    console.print("\n[bold green]切分效果：[/bold green]")
    console.print(
        f"  - 文档扩展倍数: {expansion_ratio:.2f}x (从 {clean_stats['total_docs']} 个文档扩展到 {split_stats['total_docs']} 个)"
    )
...
def show_processing_examples(
    ...
    for idx, example_idx in enumerate(indices, 1):
        ...
        console.print(
            f"[bold cyan]示例 {idx}/{len(indices)}: 文档 #{example_idx + 1}[/bold cyan]"
        )
        ...
        console.print(
            f"   - unique_id: {raw_doc.metadata.get('unique_id', 'N/A')[:16]}..."
        )
        if raw_doc.metadata.get("images_info"):
            console.print(
                f"   - 图片数: {len(raw_doc.metadata.get('images_info', []))}"
            )
        ...
        console.print("\n   [dim]内容预览:[/dim]")
        console.print(
            Panel(content_preview, border_style="green", title="原始文档内容")
        )
        ...
        if clean_doc:
            ...
            console.print(
                f"   - 字符数: {clean_stats['char_count']} ({clean_stats['char_count'] - raw_stats['char_count']:+d})"
            )
            console.print(
                f"   - Token数: {clean_stats['token_count']} ({clean_stats['token_count'] - raw_stats['token_count']:+d})"
            )
            console.print(
                f"   - 行数: {clean_stats['line_count']} ({clean_stats['line_count'] - raw_stats['line_count']:+d})"
            )
            console.print(
                f"   - unique_id: {clean_doc.metadata.get('unique_id', 'N/A')[:16]}..."
            )

            # 计算变化
            char_reduction = (
                (
                    (raw_stats["char_count"] - clean_stats["char_count"])
                    / raw_stats["char_count"]
                    * 100
                )
                if raw_stats["char_count"] > 0
                else 0
            )
            ...
            console.print("\n   [dim]内容预览:[/dim]")
            console.print(
                Panel(content_preview, border_style="yellow", title="清洗后文档内容")
            )
            ...
            if raw_doc.page_content != clean_doc.page_content:
                ...
                if raw_start != clean_start:
                    console.print("   - 开头部分已修改")
                # 检查是否有明显的格式变化
                if raw_doc.page_content.count("\n") != clean_doc.page_content.count(
                    "\n"
                ):
                    console.print(
                        f"   - 换行符数量变化: {raw_doc.page_content.count('\\n')} → {clean_doc.page_content.count('\\n')}"
                    )
        ...
        for split_doc in split_docs:
            ...
            if split_parent_id and split_doc.metadata.get(
                "source"
            ) == raw_doc.metadata.get("source"):
                # 检查是否来自同一页
        ...
        if not related_split_docs:
            for split_doc in split_docs:
                if split_doc.metadata.get("source") == raw_doc.metadata.get(
                    "source"
                ) and split_doc.metadata.get("page") == raw_doc.metadata.get("page"):
                    related_split_docs.append(split_doc)
        ...
        if related_split_docs:
            ...
            avg_chars = (
                total_chars / len(related_split_docs) if related_split_docs else 0
            )
            ...
            for i, split_doc in enumerate(related_split_docs[:3], 1):
                ...
                is_parent = split_doc.metadata.get(
                    "unique_id"
                ) == split_doc.metadata.get("parent_id")
                ...
                console.print(
                    f"   - unique_id: {split_doc.metadata.get('unique_id', 'N/A')[:16]}..."
                )
                if split_doc.metadata.get("parent_id"):
                    console.print(
                        f"   - parent_id: {split_doc.metadata.get('parent_id', 'N/A')[:16]}..."
                    )
...
def generate_processing_examples_report(
    ...
    if min_len >= num_examples:
        # 选择开头、中间、结尾的文档
        step = max(1, min_len // num_examples)
        indices = [0, min_len // 4, min_len // 2, min_len * 3 // 4, min_len - 1][
            :num_examples
        ]
    else:
        indices = list(range(min_len))
    ...
    for idx, example_idx in enumerate(indices, 1):
        ...
        if clean_doc:
            ...
            lines.append(
                f"   - Token数: {clean_stats['token_count']} ({token_diff:+d})"
            )
            lines.append(
                f"   - 行数: {clean_stats['line_count']} ({clean_stats['line_count'] - raw_stats['line_count']:+d})"
            )
            lines.append(
                f"   - unique_id: {clean_doc.metadata.get('unique_id', 'N/A')}"
            )
            ...
            if raw_doc.page_content != clean_doc.page_content:
                ...
                if raw_doc.page_content.count("\n") != clean_doc.page_content.count(
                    ...
                    lines.append(
                        f"   - 换行符数量变化: {raw_doc.page_content.count(chr(10))} → {clean_doc.page_content.count(chr(10))}"
                    )
                lines.append("")
        ...
        if related_split_docs:
            ...
            for i, split_doc in enumerate(related_split_docs, 1):
                ...
                lines.append(
                    f"   - unique_id: {split_doc.metadata.get('unique_id', 'N/A')}"
                )
                if split_doc.metadata.get("parent_id"):
                    lines.append(
                        f"   - parent_id: {split_doc.metadata.get('parent_id', 'N/A')}"
                    )
                lines.append("   - 内容:")
                lines.append("   " + "-" * 76)
            ...
            if len(related_split_docs) > 10:
                lines.append(
                    f"   ... 还有 {len(related_split_docs) - 10} 个子文档未显示"
                )
                lines.append("")
        else:
...
def generate_splitting_flow_report(
    ...
    for idx, example_idx in enumerate(indices, 1):
        ...
        for parent_doc in parent_docs:
            if parent_doc.metadata.get("source") == clean_doc.metadata.get(
                "source"
            ) and parent_doc.metadata.get("page") == clean_doc.metadata.get("page"):
                related_parent_docs.append(parent_doc)

        if related_parent_docs:
            lines.append(
                "2. m3e语义聚类后的父文档 (Parent Documents after M3E Clustering)"
            )
            lines.append(f"   - 父文档数量: {len(related_parent_docs)}")
            total_parent_chars = sum(
                len(doc.page_content) for doc in related_parent_docs
            )
            avg_parent_chars = (
                total_parent_chars / len(related_parent_docs)
                if related_parent_docs
                else 0
            )
            lines.append(
                f"   - 总字符数: {total_parent_chars} (平均: {avg_parent_chars:.1f})"
            )
            lines.append(
                "   - 说明: 清洗后的文档通过m3e模型进行语义聚类，将语义相关的段落合并为父文档"
            )
            lines.append("")
            ...
            # 3. 显示子文档
            lines.append(
                "3. 句子级切分后的子文档 (Child Documents after Sentence-level Splitting)"
            )
            ...
            if all_related_children:
                ...
                total_child_chars = sum(
                    len(doc.page_content) for doc in all_related_children
                )
                avg_child_chars = (
                    total_child_chars / len(all_related_children)
                    if all_related_children
                    else 0
                )
                lines.append(
                    f"   - 总字符数: {total_child_chars} (平均: {avg_child_chars:.1f})"
                )
                lines.append(
                    "   - 说明: 每个父文档通过句子级切分器(RecursiveCharacterTextSplitter)进行细粒度切分，生成多个子文档"
                )
                lines.append("")
                ...
                for parent_idx, parent_doc in enumerate(related_parent_docs, 1):
                    ...
                    if children:
                        lines.append(
                            f"   来自父文档 {parent_idx} 的子文档 (共{len(children)}个):"
                        )
                        ...
                        for i, child_doc in enumerate(children[:display_count], 1):
                            ...
                            lines.append(
                                f"   - unique_id: {child_doc.metadata.get('unique_id', 'N/A')}"
                            )
                            lines.append(
                                f"   - parent_id: {child_doc.metadata.get('parent_id', 'N/A')[:32]}..."
                            )
                        ...
                        if len(children) > display_count:
                            lines.append(
                                f"   ... 还有 {len(children) - display_count} 个子文档未显示"
                            )
                            lines.append("")
            else:
                lines.append(
                    "   - 未找到对应的子文档（可能父文档长度未超过阈值，直接作为最终文档）"
                )
                lines.append("")
        else:
        ...
        if related_parent_docs:
            lines.append(
                f"   - m3e聚类: 生成{len(related_parent_docs)}个父文档，共{total_parent_chars}字符"
            )
            if all_related_children:
                lines.append(
                    f"   - 句子级切分: 生成{len(all_related_children)}个子文档，共{total_child_chars}字符"
                )
                expansion_ratio = (
                    len(all_related_children) / len(related_parent_docs)
                    if related_parent_docs
                    else 0
                )
                lines.append(f"   - 父文档到子文档扩展倍数: {expansion_ratio:.2f}x")
            else:
                lines.append("   - 句子级切分: 父文档长度未超过阈值，未生成子文档")
        lines.append("")
        lines.append("")
