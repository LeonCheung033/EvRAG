...
def test_data_quality(docs: Dict[str, List[Document]]) -> Dict[str, Any]:
    ...
    for stage_name, stage_docs in docs.items():
        ...
        lengths = [len(doc.page_content) for doc in stage_docs if hasattr(doc, 'page_content')]
...
def test_index_retrieval() -> Dict[str, Any]:
    ...
    try:
        ...
        results['bm25'] = {
            ...
            'success_rate': sum(1 for r in bm25_results if r['success']) / len(bm25_results) * 100
        }
    except Exception as e:
    ...
    try:
        ...
        results['milvus'] = {
            ...
            'success_rate': sum(1 for r in milvus_results if r['success']) / len(milvus_results) * 100
        }
    except Exception as e:
...
def plot_document_length_distribution(docs: Dict[str, List[Document]], output_dir: Path):
    """绘制文档长度分布直方图"""
    print("\n" + "=" * 80)
    print("生成文档长度分布直方图")
    print("=" * 80)
    ...
    for idx, (stage_name, stage_label) in enumerate(zip(stage_names, stage_labels)):
        ...
        lengths = [len(doc.page_content) for doc in docs[stage_name] if hasattr(doc, 'page_content')]
        
        if lengths:
            ax = axes[idx]
            ax.hist(lengths, bins=50, edgecolor='black', alpha=0.7)
            ax.set_title(f'{stage_label}\n(Total: {len(lengths)}, Avg: {sum(lengths)/len(lengths):.0f})', fontsize=12)
            ax.set_xlabel('Document Length (characters)', fontsize=10)
            ax.set_ylabel('Document Count', fontsize=10)
            ax.grid(True, alpha=0.3)
            
            # 添加统计信息
            ax.axvline(sum(lengths)/len(lengths), color='red', linestyle='--', linewidth=2, label=f'Mean: {sum(lengths)/len(lengths):.0f}')
            ax.legend()
...
def plot_qa_statistics(qa_pairs: List[Dict[str, Any]], output_dir: Path):
    ...
    if question_lengths:
        ax1 = axes[0]
        ax1.hist(question_lengths, bins=30, edgecolor='black', alpha=0.7, color='skyblue')
        ax1.set_title(f'Question Length Distribution\n(Total: {len(question_lengths)}, Avg: {sum(question_lengths)/len(question_lengths):.0f})', fontsize=12)
    ...
    if answer_lengths:
        ax2 = axes[1]
        ax2.hist(answer_lengths, bins=30, edgecolor='black', alpha=0.7, color='lightcoral')
        ax2.set_title(f'Answer Length Distribution\n(Total: {len(answer_lengths)}, Avg: {sum(answer_lengths)/len(answer_lengths):.0f})', fontsize=12)
...
def plot_retrieval_comparison(index_results: Dict[str, Any], output_dir: Path):
    ...
    if not index_results.get('bm25', {}).get('success') and not index_results.get('milvus', {}).get('success'):
        print("⚠️  BM25和Milvus都未成功加载，跳过图表生成")
        return
    
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle('Retrieval Test Comparison (BM25 vs Milvus)', fontsize=16, fontweight='bold')
    ...
    if methods:
        bars = ax1.bar(methods, success_rates, color=['#3498db', '#e74c3c'], alpha=0.7, edgecolor='black')
    ...
    # 每个查询的结果数量对比
    if index_results.get('bm25', {}).get('test_results') and index_results.get('milvus', {}).get('test_results'):
        ax2 = axes[1]
        ...
        milvus_counts = [r['num_results'] for r in index_results['milvus']['test_results']]
        
        x = np.arange(len(queries))
        width = 0.35
        
        bars1 = ax2.bar(x - width/2, bm25_counts, width, label='BM25', color='#3498db', alpha=0.7, edgecolor='black')
        bars2 = ax2.bar(x + width/2, milvus_counts, width, label='Milvus', color='#e74c3c', alpha=0.7, edgecolor='black')
        
        ax2.set_title('Results per Query', fontsize=12)
        ax2.set_xlabel('Query', fontsize=10)
        ax2.set_ylabel('Result Count', fontsize=10)
        ax2.set_xticks(x)
        ax2.set_xticklabels([q[:10] + '...' if len(q) > 10 else q for q in queries], rotation=45, ha='right')
        ax2.legend()
