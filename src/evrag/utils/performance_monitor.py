...
class PerformanceMonitor:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if self.resource_samples:
            avg_cpu = sum(s.get("cpu_percent", 0) for s in self.resource_samples) / len(
                self.resource_samples
            )
            avg_memory_mb = sum(
                s.get("memory_rss_mb", 0) for s in self.resource_samples
            ) / len(self.resource_samples)
            max_memory_mb = max(
                s.get("memory_rss_mb", 0) for s in self.resource_samples
            )
        else:
            ...
            for i in range(self.gpu_count):
                ...
                if samples:
                    gpu_summary[gpu_key] = {
                        ...
                        "avg_utilization_percent": sum(
                            s[gpu_key].get("utilization_gpu_percent", 0)
                            for s in samples
                        )
                        / len(samples),
                        ...
                        "avg_memory_used_mb": sum(
                            s[gpu_key].get("memory_used_mb", 0) for s in samples
                        )
                        / len(samples),
