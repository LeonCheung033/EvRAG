...
def check_mongodb():
    ...
    try:
        ...
        # 统计父文档（无 parent_id）
        parent_count = collection.count_documents(
            {"metadata.parent_id": {"$exists": False}}
        )
        print(f"父文档数（无 parent_id）: {parent_count}")

        # 统计子文档（有 parent_id）
        child_count = collection.count_documents(
        ...
        for doc in collection.find({"metadata.parent_id": {"$exists": True}}).limit(
            1000
        ):
        ...
        if error_count == 0:
            ...
            for sample in error_samples:
                print(
                    f"  - unique_id={sample['unique_id']}, parent_id={sample['parent_id']}"
                )
        ...
        if invalid_count > 0:
            print(
                f"⚠️  有 {invalid_count} 个子文档的 parent_id 在父文档中未找到（可能是父文档长度超过阈值）"
            )
        else:
        ...
        if child_sample:
            ...
            print(
                f"子文档示例: unique_id={unique_id[:16]}..., parent_id={parent_id[:16]}..."
            )
