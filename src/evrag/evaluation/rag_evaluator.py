...
class RAGEvaluator:
    ...
    def __init__(
        ...
        if use_baseline:
            ...
            # 使用微调后的reranker模型
            reranker_path = (
                finetuned_reranker_path or settings.bge_reranker_tuned_model_path
            )
        ...
        if self._semantic_model is None:
            ...
            if torch.cuda.is_available():
                ...
                try:
                    ...
                    memory_reserved = (
                        torch.cuda.memory_reserved(semantic_gpu_id) / 1024**3
                    )  # GB

                    # 如果GPU显存使用超过40GB，认为不可用，尝试其他GPU
                    if memory_reserved > 40:
                        print(
                            f"⚠ GPU {semantic_gpu_id} 显存占用过高 ({memory_reserved:.1f}GB)，尝试其他GPU..."
                        )
                        ...
                        for gpu_id in [2, 3, 4, 5, 6, 7]:
                            ...
                            try:
                                ...
                                if memory_reserved < 40:  # 显存使用小于40GB
                                    ...
                                    print(
                                        f"✓ 找到空闲GPU: {gpu_id} (显存占用: {memory_reserved:.1f}GB)"
                                    )
                                    break
                            except:
                                continue
                            ...
                            device = "cpu"
                            print(
                                "⚠ 警告: 所有GPU显存占用过高，语义模型将使用CPU（速度较慢）"
                            )
            ...
            if self.semantic_model_path:
                self._semantic_model = SentenceModel(
                    model_name_or_path=str(self.semantic_model_path), device=device
                )
            else:
                # 使用默认模型路径
                ...
                if default_path:
                    self._semantic_model = SentenceModel(
                        model_name_or_path=str(default_path), device=device
                    )
                else:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "无法加载语义相似度模型，请指定semantic_model_path"
                        )
    ...
    def evaluate_single(
        ...
        milvus_docs = self.milvus_retriever.retrieve_topk(
            question, topk=self.milvus_topk * 2
        )
        ...
        timings["retrieval_time"] = (
            timings["bm25_time"] + timings["milvus_time"] + timings["merge_time"]
        )
    ...
    def calculate_summary_metrics(self) -> Dict[str, Any]:
        ...
        # 计算平均值
        avg_semantic_keyword_score = (
            float(np.mean(semantic_keyword_scores)) if semantic_keyword_scores else 0.0
        )
        ...
        summary = {
            ...
            "response_time": {
                ...
                "mean_reranking_time": float(
                    np.mean([r.get("reranking_time", 0.0) for r in response_times])
                )
                if response_times
                else 0.0,
