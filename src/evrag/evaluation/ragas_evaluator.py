...
class RAGasEvaluator:
    ...
    def __init__(
        ...
        self.api_key = api_key or os.environ.get("SILICONFLOW_API_KEY") or settings.siliconflow_api_key
        self.base_url = base_url or os.environ.get("SILICONFLOW_BASE_URL") or settings.siliconflow_base_url
        # 默认使用DeepSeek-V3模型（通过SiliconFlow）
        self.model_name = model_name or "deepseek-ai/DeepSeek-V3"
        
        if not self.api_key:
            raise ValueError("RAGas评估需要API密钥，请设置api_key参数或SILICONFLOW_API_KEY环境变量")
        if not self.base_url:
            raise ValueError("RAGas评估需要API基础URL，请设置base_url参数或SILICONFLOW_BASE_URL环境变量")
        if not self.model_name:
            raise ValueError("RAGas评估需要模型名称，请设置model_name参数或RAGAS_MODEL_NAME环境变量")
    ...
    def evaluate_from_rag_results(
        ...
        if len(dataset) > 0:
            sample = dataset[0]
            print(f"调试信息 - 第一条样本格式:")
            print(f"  user_input类型: {type(sample.get('user_input'))}, 长度: {len(sample.get('user_input', ''))}")
            print(f"  retrieved_contexts类型: {type(sample.get('retrieved_contexts'))}, 数量: {len(sample.get('retrieved_contexts', []))}")
            if sample.get('retrieved_contexts'):
                print(f"  第一个context长度: {len(sample.get('retrieved_contexts')[0])}")
            print(f"  response类型: {type(sample.get('response'))}, 长度: {len(sample.get('response', ''))}")
            print(f"  reference类型: {type(sample.get('reference'))}, 长度: {len(sample.get('reference', ''))}")
        ...
        try:
            ...
            if precision_col:
                ...
                if len(valid_precision) > 0:
                    ...
                    print(f"✓ {precision_col}计算成功: {len(valid_precision)}/{len(result_df)}个有效值")
                else:
                    context_precision = 0.0
                    ...
                    if "error" in result_df.columns:
                        ...
                        if error_count > 0:
                            ...
                            print(f"  错误示例: {result_df[result_df['error'].notna()]['error'].iloc[0]}")
            else:
                print(f"⚠ 警告: 未找到context_precision相关列")
                print(f"  可用列: {list(result_df.columns)}")
                print(f"  尝试查找包含'precision'的列: {[col for col in result_df.columns if 'precision' in col.lower()]}")
                # 尝试使用其他可能的列名
                for alt_col in ["precision", "context_precision_score", "llm_context_precision"]:
    ...
    def evaluate_from_dataset(
        ...
        try:
            ...
            if "context_recall" in result_df.columns:
                ...
                context_recall = float(recall_mean) if not np.isnan(recall_mean) else 0.0
            
            if "context_precision" in result_df.columns:
                precision_mean = result_df["context_precision"].mean()
                context_precision = float(precision_mean) if not np.isnan(precision_mean) else 0.0
