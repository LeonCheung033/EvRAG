...
def calculate_semantic_keyword_score(
    ...
    semantic_score = semantic_search(
        semantic_model.encode([gold]),
        semantic_model.encode([pred]),
        top_k=1
    )[0][0]['score']
    
    # 计算关键词匹配得分
    join_keywords = [word for word in keywords if word in pred]
    keyword_score = _calc_jaccard(join_keywords, keywords, threshold=0.3) if keywords else None
...
def _calculate_rouge_chinese(pred: str, gold: str) -> Dict[str, float]:
    ...
    rouge_l = _calculate_lcs(pred_tokens, gold_tokens) / len(gold_tokens) if gold_tokens else 0.0
