...
class SFTDataAnalyzer:
    ...
    def _analyze_summary_items(
        self, items: List[Dict[str, Any]], split_name: str
    ) -> Dict[str, Any]:
        """分析summary数据项"""
        if not items:
            return {"count": 0}
    ...
    def _plot_summary_distribution(
        self, summary_stats: Dict[str, Any], output_dir: Path
    ):
        """绘制SFT数据样本分布图"""
        ...
        ax2.text(
            0, train_avg, f"{train_avg:.1f}", ha="center", va="bottom", fontsize=10
        )
        ...
        ax4.bar(
            x + width / 2,
            [train_no_answer, test_no_answer],
            width,
    ...
    def _plot_rerank_label_distribution(
        self, rerank_stats: Dict[str, Any], output_dir: Path
    ):
        """绘制Reranker数据标签分布图"""
        if not rerank_stats:
            return
...
class SFTDataValidator:
                ...
                for line_num, line in enumerate(f, 1):
                    ...
                    try:
                        ...
                        if not isinstance(item["content"], str):
                            ...
                            continue
                        if not isinstance(item["label"], int) or item["label"] not in [
                            0,
                            1,
                            2,
                        ]:
    ...
    def validate_data_balance(self) -> Dict[str, Any]:
        ...
        for split in ["train", "dev", "test"]:
            ...
            # 计算比例
            rates = {label: count / total for label, count in label_counts.items()}
            ...
            with open(train_data_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        if not all(
                            key in item
                            for key in ["query", "context", "response", "merged_docs"]
                        ):
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        try:
                            item = json.loads(line)
                            if not all(
                                key in item for key in ["query", "content", "label"]
                            ):
                                missing_fields_count += 1
                        except:
                            pass
    ...
    def print_validation_results(self, results: Dict[str, Any]):
        ...
        if train_data_result.get("valid"):
            console.print(
                f"[green]✓[/green] train_data.json: 格式正确 ({train_data_result.get('valid_count', 0)}/{train_data_result.get('total_count', 0)} 条有效)"
            )
        else:
            console.print("[red]✗[/red] train_data.json: 格式错误")
        ...
        for split in ["train", "test"]:
            ...
            if split_result.get("valid"):
                console.print(
                    f"[green]✓[/green] summary_data/{split}.json: 格式正确 ({split_result.get('valid_count', 0)}/{split_result.get('total_count', 0)} 条有效)"
                )
            else:
        ...
        for split in ["train", "dev", "test"]:
            ...
            if split_result.get("valid"):
                console.print(
                    f"[green]✓[/green] rerank_data/{split}.json: 格式正确 ({split_result.get('valid_count', 0)}/{split_result.get('total_count', 0)} 条有效)"
                )
            else:
        ...
        for split in ["train", "dev", "test"]:
            ...
            if split_result:
                if split_result.get("balanced"):
                    console.print(
                        f"[green]✓[/green] rerank_data/{split}.json: 数据平衡"
                    )
                else:
                    console.print(
                        f"[yellow]⚠[/yellow] rerank_data/{split}.json: 数据不平衡"
                    )
                    for warning in split_result.get("warnings", []):
                        console.print(f"  - {warning}")
                label_rates = split_result.get("label_rates", {})
                console.print(
                    f"  标签分布: 正样本={label_rates.get(2, 0) * 100:.1f}%, 中等样本={label_rates.get(1, 0) * 100:.1f}%, 负样本={label_rates.get(0, 0) * 100:.1f}%"
                )
        ...
        if train_data_complete.get("complete"):
            console.print("[green]✓[/green] train_data.json: 数据完整")
        else:
            console.print(
                f"[red]✗[/red] train_data.json: {train_data_complete.get('missing_fields', 0)} 条数据缺少字段"
            )

        for split in ["train", "test"]:
            summary_complete = completeness_result.get("summary_data", {}).get(
                split, {}
            )
            if summary_complete.get("complete"):
                ...
                console.print(
                    f"[red]✗[/red] summary_data/{split}.json: {summary_complete.get('missing_fields', 0)} 条数据缺少字段"
                )
        ...
        for split in ["train", "dev", "test"]:
            ...
            if rerank_complete.get("complete"):
                ...
                console.print(
                    f"[red]✗[/red] rerank_data/{split}.json: {rerank_complete.get('missing_fields', 0)} 条数据缺少字段"
                )
