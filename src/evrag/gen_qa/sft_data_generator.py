    ...
    def generate_train_data(
        ...
        def process_qa_pair(qa_pair: Dict[str, Any]) -> Optional[Dict[str, Any]]:
            ...
            for attempt in range(max_retries):
                try:
                    # RAG检索
                    bm25_docs = self.bm25_retriever.retrieve_topk(
                        query, topk=self.bm25_topk
                    )
                    milvus_docs = self.milvus_retriever.retrieve_topk(
                        query, topk=self.milvus_topk
                    )
                    merged_docs = merge_docs(
                        bm25_docs, milvus_docs, mongodb_client=self.mongodb_client
                    )
                    ranked_docs = self.reranker.rank(
                        query, merged_docs, topk=self.reranker_topk
                    )
                    ...
                    response = re.sub(
                        r"^答案内容[：:]\s*", "", response, flags=re.MULTILINE
                    )
                    ...
                    if is_connection_error and attempt < max_retries - 1:
                        ...
                        print(
                            f"[WARNING] Connection error for query '{query[:50]}...': {error_str[:100]}"
                        )
                        print(
                        ...
                        if is_connection_error:
                            print(
                                f"[ERROR] Connection error for query '{query[:50]}...': {error_str[:100]}, failed after {max_retries} retries"
                            )
                        else:
                            print(
                                f"[ERROR] Error processing query '{query[:50]}...': {error_str[:100]}"
                            )
                        return None
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            # 写入结果
            with open(
                output_path, "a" if checkpoint_file.exists() else "w", encoding="utf-8"
            ) as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    for cite in all_cites:
                        ...
                        cite = cite.replace(",", "，")
                        cite_numbers = [
                            int(k) for k in cite.split("，") if k.strip().isdigit()
                        ]
                    ...
                    # 去除 "答案内容：" 前缀（如果存在）
                    answer = re.sub(
                        r"^答案内容[：:]\s*", "", answer, flags=re.MULTILINE
                    ).strip()
                    ...
                    # 构建instruction
                    instruction = LLM_CHAT_PROMPT.format(
        ...
        with open(train_data_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    format_answer = (
                        answer + f"【{','.join([str(c) for c in cites])}】"
                        if cites
                        else "无答案"
                    )
                    ...
                    neg_docs = [
                        doc for doc in merged_docs_list if doc not in context_list
                    ]
                    ...
                    if format_answer != "无答案":
                        if is_test:
                            ...
                            if context_list:
                                ...
                                if len(context_list) >= 2:
                                    content_list.append(
                                        random.choice(context_list[-2:])
                                    )
                                if neg_docs:
                                    content_list.append(random.choice(neg_docs))
                                # 测试集格式：{"query": "...", "content": [...]}
                                rerank_test.append(
                                    {"query": query, "content": content_list}
                                )
                        else:
                            # 训练集：生成正样本、中等样本、负样本
                            if context_list:
                                ...
                                rerank_train.append(
                                    {"query": query, "content": positive, "label": 2}
                                )

                                if len(context_list) >= 2:
                                    middle = random.choice(context_list[-2:])
                                    rerank_train.append(
                                        {"query": query, "content": middle, "label": 1}
                                    )
                        ...
                        if not is_test:
                            ...
                            if merged_docs_list:
                                ...
                                rerank_train.append(
                                    {"query": query, "content": negative, "label": 0}
                                )
        ...
        def process_qa_pair(qa_pair: Dict[str, Any]) -> Optional[Dict[str, Any]]:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    ranked_docs = self.reranker.rank(
                        query, merged_docs, topk=reranker_k
                    )
