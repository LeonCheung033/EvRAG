...
class QAProcessor:
    ...
    def load_filtered_qa_pairs(self, filtered_path: Path) -> List[Dict[str, Any]]:
        ...
        if not filtered_path.exists():
            raise FileNotFoundError(f"Filtered QA pairs file not found: {filtered_path}")
                ...
                for line in fd:
                    ...
                    try:
                        ...
                        if question and raw_resp:
                            expand_questions = self.qa_generator.parse_generalized_questions(raw_resp)
                            expand_qa_pairs[question] = expand_questions
                    except Exception as e:
                        print(f"Error parsing expand_qa_pair line: {e}")
                        continue
            print(f"Loaded {len(expand_qa_pairs)} expanded questions from {output_file}")
    ...
    def add_negative_samples(
        ...
        if not negative_samples_path.exists():
            print(f"Warning: {negative_samples_path} does not exist, skipping negative samples")
            return train_qa_pairs, test_qa_pairs
        ...
        print(f"Added negative samples: train: {len([q for q in train_qa_pairs if q['answer'] == '无答案'])}, "
