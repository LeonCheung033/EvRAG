...
本文档记录了微调后模型与基线模型的性能对比评估结果，包括Reranker模型和LLM模型的详细对比分析。
...
- **基线模型**: BGE-Reranker-v2-m3（未微调）
- **微调后模型**: `models/finetuned/bge_reranker/model`
- **评估数据集**: `data/rerank_data/dev.json`（1000条数据）
- **评估查询数**: 350个
- **平均每个查询的文档数**: 2.86个
...
|------|---------|-----------|---------|---------|
| **NDCG@10** | 0.7520 | **0.7906** | +0.0385 | **+5.12%** ✅ |
| **MRR** | 0.8086 | **0.8229** | +0.0143 | **+1.77%** ✅ |
| **Precision@10** | 0.1857 | 0.1857 | 0.0000 | 0.00% |
| **Recall@10** | 0.9286 | 0.9286 | 0.0000 | 0.00% |
...
1. **NDCG@10提升5.12%**
   - 从0.7520提升到0.7906
   - **意义**: 排序质量显著提升，相关文档的排名更靠前
   - **重要性**: 这是最重要的改进指标，因为NDCG综合考虑了排序位置和相关性
   - **实际影响**: 用户看到的搜索结果排序更合理，相关文档更容易被看到

2. **MRR提升1.77%**
   - 从0.8086提升到0.8229
   - **意义**: 第一个相关文档的排名更靠前
   - **重要性**: 对于用户来说，这意味着更快的找到答案
   - **实际影响**: 用户更快找到第一个相关文档，减少浏览时间
...
1. **Precision@10未变化**
   - 基线模型和微调后模型都是0.1857
   - **可能原因**:
     - 每个query平均只有2.86个文档，Top-10的precision计算可能不够敏感
     - 数据集中相关文档的比例相对固定
     - Precision@10的计算方式可能对排序变化不敏感

2. **Recall@10未变化**
   - 基线模型和微调后模型都是0.9286（已经很高）
   - **说明**:
     - 基线模型的召回率已经接近完美（92.86%）
     - 微调主要提升了排序质量，而不是召回率
     - 没有因为微调而降低召回率，仍然能覆盖大部分相关文档
...
- NDCG@10提升5.12%是显著的改进
- MRR提升1.77%说明第一个相关文档更容易被找到
- 这些改进在实际应用中会带来更好的用户体验

#### 为什么Precision和Recall未变化？
...
2. **Precision@10的计算特点**
   - 每个query平均只有2.86个文档
   - Top-10的precision可能对排序变化不够敏感
   - 如果相关文档数量少，precision可能保持稳定

3. **数据特点**
   - 评估集有350个查询
   - 每个查询平均2.86个文档对
   - 数据规模较小，可能限制了某些指标的敏感性

### 1.5 实际应用意义

**NDCG和MRR的提升在实际应用中非常重要：**

- **NDCG提升5.12%**: 用户看到的搜索结果排序更合理，相关文档更容易被看到
- **MRR提升1.77%**: 用户更快找到第一个相关文档，减少浏览时间
- **Recall保持0.9286**: 说明微调没有降低召回率，仍然能覆盖大部分相关文档

### 1.6 改进最大的查询示例

根据评估结果，以下查询的NDCG改进最大（从0.5869提升到1.0000，改进+0.4131）：

1. "当APP_w048辅助驾驶功能暂时不可用警报出现时，建议采取什么措施？"
2. "行程安排中，怎么取消或重新加入超级充电站这个点？"
3. "什么情况下行程安排会弹出提醒或警示？"
4. "如果房子的电线发热严重，该联系哪个部门？"
5. "轻触地图上的充电点标识能进行哪些操作？"

这些查询的改进说明微调后的模型在处理特定类型的查询时表现更好。
...
BASELINE_PORT: 8000
FINETUNED_PORT: 8001
BASELINE_GPU_IDS: 0,1
FINETUNED_GPU_IDS: 2,3
```
